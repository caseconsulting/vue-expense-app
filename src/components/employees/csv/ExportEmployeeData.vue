...
function filterEmployees(employees) {
  ...
  return _.filter(employees, (e) => {
    ...
    if (f.period != 'All') {
      ...
      let deptYearValid = !this.deptDate || isSameOrAfter(e.deptDate, f.period, exportType.value.periodType);
    ...
    for (let i = 0; i < f.tags.length; i++) {
      ...
      if (tagFlip.value.includes(tag.id) && !tagHasEmployee) employeeHasTag = true; // tag is negated and employee is on it
      if (!tagFlip.value.includes(tag.id) && tagHasEmployee) employeeHasTag = true; // tag is normal (not negated) and employee is not on it
...
function removeTag(item) {
  const selIndex = filters.value['tags'].findIndex((t) => t.id === item.id);
  if (selIndex >= 0) {
    filters.value['tags'].splice(selIndex, 1);
  }
} // remove
