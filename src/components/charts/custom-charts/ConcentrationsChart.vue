  ...
  option.value = {
    ...
    onClick: (x, y) => {
      if (_first(y)) {
        ...
        localStorage.setItem('requestedFilter', getConcentrationList(labelClicked, degree.value));
        router.push({
          path: '/employees',
          name: 'employees'
        });
 ...
 *
 * @param concentration the concentration that needs to be searched
 * @param degreeType the type of degree (bachelor, master, etc.)
 */
function getConcentrationList(concentration, degreeType) {
  ...
  empList = _filter(employees, (emp) => {
    return _some(emp?.education, (edu) => {
      return _some(edu?.degrees, (deg) => {
        return (
          (deg.degreeType === degreeType || deg.degreeType === degreeType + 's') &&
          deg.concentrations?.includes(concentration)
        );
      });
    });
  });
